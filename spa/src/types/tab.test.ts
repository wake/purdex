...
describe('createTab', () => {
  it('creates a tab with session content', () => {
    const tab = createTab({ kind: 'session', sessionCode: 'abc123', mode: 'terminal' })
    ...
    if (tab.layout.type === 'leaf') {
      ...
      expect(tab.layout.pane.content).toEqual({ kind: 'session', sessionCode: 'abc123', mode: 'terminal' })
    }
  })
...
describe('isStandaloneTab', () => {
  it('returns true when tab is not in any workspace', () => {
    const tab = createTab({ kind: 'session', sessionCode: 'abc', mode: 'terminal' })
  ...
  it('returns false when tab is in a workspace', () => {
    const tab = createTab({ kind: 'session', sessionCode: 'xyz', mode: 'stream' })
