...
```
9.1 Daemon FS ──→ 9.2 SPA API ──→ 9.4 Monaco Editor
                                  ──→ 9.6 File Tree
                                  ──→ 9.8 Grep

9.3 Opener Registry ──→ 9.5 Openers 註冊
                      ──→ 9.6 File Tree（開檔行為）

9.4 Monaco Editor ──→ 9.7 大檔案處理
                   ──→ 9.9 Diff View

9.5 Openers ──→ 9.7 二進制偵測
```

可並行的組合：
- 9.1（daemon）與 9.3（registry）無依賴，可並行
- 9.8（grep）與 9.9（diff）無互依賴，可並行
...
- 9.1：Go test（API handler + 路徑驗證 + 權限控制）
- 9.2：API 封裝 unit test（request 格式、error handling）
- 9.3：registry CRUD + match + priority 排序
- 9.4：editor pane 渲染 + save flow + dirty state
- 9.5：各 opener match 規則 + open 行為
- 9.6：file tree 展開/收合 + CRUD 操作 + opener 整合
- 9.7：大檔案警告觸發 + 二進制偵測準確性
- 9.8：grep 結果渲染 + 點擊跳轉
- 9.9：diff view 渲染 + 模式切換
