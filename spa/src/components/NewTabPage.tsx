...
export function NewTabPage({ onSelect }: Props) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-start pt-16 gap-8 overflow-y-auto">
      <h2 className="text-lg text-text-secondary">New Tab</h2>
      {providers.length === 0 && (
        <p className="text-sm text-text-secondary">No content providers registered</p>
      )}
      {providers.map((p) => (
        <section key={p.id} className="w-full max-w-md">
          <h3 className="text-sm font-medium text-text-secondary mb-2 px-2">{p.label}</h3>
