...
> **Status**：v6.2（PR R5 standard codex review F6 zero-emit rearm tight-loop reframe — F7 三 commit 落地）。v5 contract 主軸不變：**2-phase + 2-case truth table contract** — Phase A 等 `ScreenStable`（pane 停下來進入穩定）/ Phase B 監控 `ScreenChanged` → emit-once；Phase A 失敗一律歸 case 2 by contract → 由 PdxStop hook 自然 cover，detector 不再嘗試補位。實作面 — `armed atomic.Bool`（Phase A→B 閘門）+ `emitted atomic.Bool` + `sync.Mutex` + **`closed bool` mutex-protected**（v6.1 round 6 P1 fix — detector return 與 cb in-flight 互斥防 close-race）；isCodexAlive 用 `prober.FirstAliveAgentInTree(paneID)`（已用 ActivePanePID exact resolve；修 round 3 F3）；emit 前 mutex 內 double-check isCodexAlive（修 round 3 F2 race window）。撤掉 grace gate / `now func()` seam — Phase A 由 ScreenStable 自然 demarcate。**v6 update**：round 5 抓到的 reject path race 由 J3 PR #797 dispatcher 雙向 `probeIntentPreGraceWindow=300ms` + `classifyAsHookRace` helper 接管（per W6-3 §9.14 + fix-spec §3 generic-Kind 不特化原則）；本 PR detector 仍是 dumb emit；§0.5 + R14 + §8 anchor + A9/A10 mlab gate 落地。**v6.1 update**：round 6 P1 抓到 watchLoop callback inline sync call 與 wrap `close(out)` race；detector 內 mutex 加 `closed bool` flag 統一保護 close/send 互斥（§2.1 + §2.3 R15 + §4.3 + §6 P1-T2/T3）。**v6.2 update（PR R5 → F7）**：R5 standard codex review 抓到 R4 F6 `WatchHandle.Done()` detector lifecycle observation 在持續性 tmux failure 下變 tight rearm loop（detector zero-emit return → dispatcher !appliedAny teardown → applyStatus rearm → next Watch baseline-fail → repeat，由 J3 pre-grace 300ms 兜底但仍是病態 churn）。F7 將 baseline 失敗從 detector lifecycle event 收回 probe layer 作 transient state 處理（watchLoop 改 baseline retry pattern；detector 撤回 `case <-wh.Done()` arm；`WatchHandle.Done()` API 保留作 teardown observability hygiene 但不再驅動 detector 狀態機）。三 commit：F7-1 probe baseline retry；F7-2 detector revert + 新 `TransientBaselineFailureRecoversAndEmits` regression pin；F7-3 spec §0.4 + §4.3 + §8 reframe + 3 drift anchor。
>
...
> - PR R5 P2（R4 F6 `case <-wh.Done()` zero-emit rearm tight-loop：watchLoop baseline-fail self-cleanup + close(done) → detector observe Done 立刻 zero-emit return → dispatcher !appliedAny teardown → applyStatus 同 OnEntryStatus rearm → 新 Watch 立刻又 baseline-fail，重複；J3 pre-grace 300ms 是被動兜底而非設計意圖，CPU spin 與 log noise 高）→ ✅ fixed in v6.2 (F7) — baseline-fail 是 probe-layer transient state 不是 detector lifecycle event；watchLoop 改 baseline retry pattern（map entry 不清、Done() 不關、loop 不退出，每 tick 都當下一次 baseline 嘗試）；detector 撤回 `case <-wh.Done()` arm；`WatchHandle.Done()` API 保留作 teardown observability hygiene；replaces R2 baseline-fail-self-cleanup contract 與 R4 F6 detector lifecycle observation
>
...
**Baseline capture failure semantics（v6.2 R5 F7 contract reframe）**：

Probe baseline capture 是 watcher 內部 baseline 建立步驟，不是 ScreenChange detector lifecycle event。watchLoop 啟動時會立即嘗試 capture baseline；若成功，happy path timing 與既有 `Prober.Watch` 一致：後續 tick 開始比較，`IdleStableTicks=3` 約 1.5s 後可產生 `ScreenStable`。**若立即 baseline capture 遇到 transient tmux error**（pane 剛關、server hiccup、capture-pane 一時 unresponsive），watchLoop **不退出、不清 watcher map entry、不關 `WatchHandle.Done()`**；它進入 500ms poll loop，直到第一次成功 capture，並把該次成功 capture 視為 baseline（seeding tick 不 emit ScreenChanged，因為沒有上一輪 hash 可比較）。這讓 baseline-fail path 與 mid-loop tmux-error path **對稱**：tmux error 只 skip tick，不產生 false event，也不改 detector lifecycle。

歷史脈絡：

- v6.0 之前（R2 fix）：baseline-fail self-cleanup map entry + return — 為了讓 `HasWatcher` 不報 stale live watcher。
- v6.1 R4 F6：在 R2 之上加 `WatchHandle.Done()` close-on-watchLoop-exit 通道；detector 主 goroutine `case <-wh.Done():` 觀察 → zero-emit return → dispatcher `!appliedAny` teardown → applyStatus 同 OnEntryStatus rearm。
- v6.2 R5（F7 fix）：R5 codex review 抓到 R4 F6 在持續性 tmux failure 下變 tight rearm loop（baseline-fail → detector zero-emit → teardown → rearm → 新 Watch 立刻 baseline-fail → 重複；J3 pre-grace 300ms 是被動兜底而非設計意圖）。F7 撤回 R2 self-cleanup 與 R4 F6 detector observation：baseline 失敗收回 probe layer 作 transient state，watchLoop 自己 retry；detector lifecycle 只看 emit / ctx cancel；`WatchHandle.Done()` API 保留作 teardown observability hygiene（用於 ownership-aware StopWatchOwned 測試 + sweep / orchestrator paths 觀察 watchLoop exit），但**不**驅動 detector 狀態機。
...
**Detector lifecycle vs `WatchHandle.Done()`（v6.2 R5 F7 contract）**：

`StartScreenChangeDetector` 主 goroutine 只觀察 **probe callback 事件**（`emittedCh` close — Phase B 成功 emit 觸發）與 **`ctx.Done()`**（dispatcher 因 status 變化 cancel）。Phase A 由 `ScreenStable` arm，Phase B 由 `ScreenChanged` + `isCodexAlive()` emit-once；**`WatchHandle.Done()` 不參與 ScreenChange detector 的狀態機**。

`Done()` 是 probe teardown hygiene：用於測試與未來觀察 `StopWatch` / `StopWatchOwned` / replacement / `StopAll` / ctx cancel 導致的 watchLoop exit，不代表 baseline failure，也不是 detector re-arm signal。將 `Done()` 解讀為「watcher 死了、detector 該放棄」會違反 v6.2 contract — 如 R4 F6 嘗試所示，造成 zero-emit rearm tight loop（baseline 失敗 → detector return → dispatcher teardown+rearm → 新 Watch 立刻 baseline 失敗 → 重複）。F7 已將 `case <-wh.Done()` 從 detector 主 select 撤回；GoDoc 內保留 `WatchHandle.Done()` API 說明但只供 ownership-aware StopWatchOwned 測試與 sweep / orchestrator path 使用。

baseline 失敗的 retry 行為由 `internal/agent/probe/activity.go` watchLoop 內部負責（**every tick before first successful capture is treated as another baseline attempt**），不會穿透到 detector 層；detector 只會看到第一次成功 capture 之後的正常 ScreenStable / ScreenChanged 事件流。
...
| 想讓 ScreenChange detector select `WatchHandle.Done()` 作為零 emit 退場 | drift：baseline capture error 已被定義為 probe-layer transient tick，不是 detector lifecycle。detector 只應由 `ScreenStable` / `ScreenChanged` / `ctx.Done()` 驅動。R4 F6 曾加過此分支，被 R5 finding 抓到造成 tight rearm loop（detector zero-emit return → dispatcher !appliedAny teardown → applyStatus rearm → 新 Watch baseline-fail → 重複），F7 已 revert。`Done()` API 保留作 teardown observability hygiene 但不能驅動 detector 狀態機。 |
| 想把 baseline capture fail 恢復成 `cleanup()+return` | drift：會重新製造 active intent 還活著但 watcher 死掉的跨層補丁需求；正確 contract 是 baseline 與 mid-loop tmux error 對稱 skip/retry — 兩者都不退出 watchLoop、不清 map entry、不關 Done()。F7 fix 已落 watchLoop baseline retry pattern（每 tick 直到第一次成功 capture，把該次 capture 視為 baseline，seeding tick 不 emit）。撤回此 retry 等於回到 R2/R4 F6 contract，會重新觸發 R5 抓到的 tight rearm loop。 |
| 想把 `WatchHandle.Done()` 解讀為 ScreenChange lifecycle signal | drift：`Done()` 只表示 watchLoop teardown hygiene（StopWatch / StopWatchOwned / replacement / StopAll / ctx cancel / shutdown）；不能用來推論 approval / reject / Phase A failure / detector 該 retry。F7 後 detector lifecycle 與 watcher lifecycle 解耦，由 probe 內部 retry 隔絕 baseline-fail 噪音。 |
