# Spec — Lights L2: Proxy detach on Stop (v5 — wording polish + cc regression coverage + PreToolUse no-parent guard)

**Kickoff**: `kickoff_codex_broker_and_lights_governance.md` §燈號 L2
**Date**: 2026-05-01 (v1) / 2026-05-02 (v2, v3, v4, v5)
...
**v5 supersedes v4** — v4 was blocked by Round-5 codex review (`task-monbbgih-2pcw1y`) with 5 non-blocker findings (3 medium / 2 low, all wording / coverage / no architectural change). v5 fixes all 5: F1 AC5 rg pattern (escape mistake from markdown render), F2 row 17 wording + new cc regression row 17b, F3 PreToolUse no-parent must skip rather than fall through to generic frame creation, F4 §7 stray "Handled" wording, F5 LOC unification. **No design changes** — v5 is the final spec before implementation. Review history: v1 → block (`task-mon01q2i-i88vwf`) → v2 → block (`task-mon4zbs3-ceswha`) + F2 consult (`task-mon9h5l9-byczvd`) → v3 → block (`task-monase7h-0jalqc`) → v4 → block-no-blocker (`task-monbbgih-2pcw1y`) → v5.

**Estimated size**: ~525-735 raw LOC / ~380-550 effective LOC (counting comment lines once). Total PR diff cap (AC6) ≤ 850 lines including spec + plan + tests. Single-PR but mid-large — use two-round codex PR review (standard + 3-parallel adversarial).
...
**C.1 PreToolUse no-parent guard (F3/v5 fix)**

When the new lifecycle case runs for a `PdxPreToolUse` event with `frame == nil` AND `findProxyParent(req)` returns nil (no proxy parent in the pane), the case must return early with trace `Decision: "skipped"`, `Reason: "pre_tool_without_proxy_parent"` — and NOT fall through to the generic post-switch frame-creation path (`frame_ops.go:251` onward, which sets `status = StatusIdle` at `frame_ops.go:303` and would create a standalone idle frame at `frame_ops.go:475-485`). PreToolUse is detail-only by design (§3.3.C Part 2 returns `Status=""`); a fall-through would silently materialize a frame with synthetic idle status, contradicting "broker is starting work for a turn" semantics. UserPromptSubmit's existing `Status: StatusRunning` derive (`status.go:18`) makes its no-parent fall-through behavior unchanged from current code — only PreToolUse needs this guard.
...
3. If found: mutate `refs[i].SourceTurnID = newTurnID` and `refs[i].StartedAt = broadcastTs` (refresh recency). **Parse-failure semantics**: when `newTurnID == ""` (incoming raw_event was malformed, missing `turn_id`, or over-cap), the field is unconditionally overwritten to empty rather than preserving the previous turn — see §3.5 known-limitation #5 for the round-2/round-3 derivation.
...
5. **Parse-failed upsert downgrades existing SourceTurnID** — when `parseCodexTurnID` returns "" (malformed/missing/over-cap raw_event) and an upsert finds an existing ref, the ref's `SourceTurnID` is unconditionally overwritten to empty rather than preserving the previous turn. This is a deliberate spec choice on a physical-tradeoff boundary inside the single `SourceTurnID` field — neither preserve nor overwrite is universally correct because the parse-failed event carries no information about whether the broker has switched to a new turn. **Rationale**: preserving the previous turn would let a legitimate late `Stop(t_old)` arriving at the next turn's boundary targeted-detach the still-active ref; that race is part of the normal codex CLI traffic pattern (every turn boundary). Overwriting to empty accepts the rarer **double-malformed** race instead — when both the upsert AND a subsequent empty-turn Stop are malformed, the empty-turn Stop's case (c) detach (turn-aware filter, §3.3.D) will match and drop the ref. **Observable signal**: trace reason `proxy_subagent_detached_on_stop` where the prior upsert chain shows an empty-turn UserPromptSubmit/PreToolUse on the same broker. **Future improvement** (out of scope here): extend the schema with explicit unknown/new-generation state (e.g. `SourceTurnState` enum or `epoch` counter, or a daemon-local sentinel that cannot match any real turn_id) so neither race is exposed; tracked as a follow-up.
...
| 17 | parent cc + 0 refs, opencode UserPromptSubmit from PID=99, AgentType=opencode (no codex turn_id) | new lifecycle case **breaks early** at `req.AgentType == "codex"` gate (B2/v4); execution continues after the switch into the **pre-existing generic frame path** (`frame_ops.go:251` onward — SessionStart proxy fast-path at `frame_ops.go:210` is gated to `LifecycleSessionStart` so does NOT run for UserPromptSubmit). For frame == nil + no proxy parent: existing path may create a standalone frame with the derive result's status; for frame != nil: narrow `UpdateHookPath` runs. **Must NOT write SourceTurnID, must NOT mutate parent.Subagents.** Existing opencode behavior unchanged from baseline. (F2/v5 fix: v4 mis-described this as falling through to fast-path) | §2.5 opencode unchanged isolation guard |
| 17b | parent cc frame exists for PID=10, cc UserPromptSubmit from PID=10, AgentType=cc | new lifecycle case breaks early at `req.AgentType == "codex"` gate; execution continues into existing narrow `UpdateHookPath` (`frame_ops.go:440` onward); frame status updates per cc derive (typically `StatusRunning` from `cc/status.go`); `Subagents` unchanged; no `SourceTurnID` written; no proxy detach attempted | F2/v5 cc UserPromptSubmit regression guard (the missing row in v4) |
...
| 20 | pane has 0 frames + 0 refs | PreToolUse from PID=42 with no proxy parent (`findProxyParent` returns nil), AgentType=codex | new lifecycle case returns early with `Decision: "skipped"`, `Reason: "pre_tool_without_proxy_parent"`. **No frame created, no Subagents mutation, no status broadcast** (specifically: `m.frames` count unchanged before/after; no `frame_apply` log line for new frame; no `currentStatus[tmuxSession]` write). | F3/v5 PreToolUse no-parent guard — guards against v4 spec gap where Status="" + no-parent could fall through to generic path and create idle standalone frame at `frame_ops.go:475-485` |
...
| Catalog change for `PdxPreToolUse` (`HookHandlingUnsupported` → omitted Handling, defaults to `HookHandlingDetail` via `EffectiveHookHandling`; F4/v5 fix) breaks existing code paths | grep `HookHandlingUnsupported` and `LifecycleNone` consumers in `internal/agent/codex/` to confirm no path depends on `PdxPreToolUse` being unsupported. AC8 enforces specific grep list. |
...
| **AC5** | Trace reason vocabulary stays additive. PR review must run this complete alternation `rg` (F1/v5 fix — v4 had backslash-escaped pipes which ripgrep treats as literal pipe characters, making the pattern silently match nothing) and confirm matches are exactly the §7-listed reasons: <br>`rg '"(proxy_subagent_attached\|proxy_subagent_detached\|proxy_subagent_detached_on_stop\|proxy_subagent_detached_on_stop_turn\|proxy_subagent_stop_no_match\|proxy_subagent_stop_parse_failed\|proxy_subagent_upserted_on_user_prompt\|proxy_subagent_attached_on_user_prompt\|user_prompt_without_proxy_parent\|pre_tool_without_proxy_parent)"' --type go` <br><br>**IMPORTANT**: when running this command, the `\|` characters above must be entered as raw `|` (unescaped pipe) in your shell. The escaping shown here is markdown-table-cell artifact only. The functional pattern in shell is: `"(proxy_subagent_attached|proxy_subagent_detached|...)"` |
