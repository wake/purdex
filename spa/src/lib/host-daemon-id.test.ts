...
describe('startHostDaemonIdVerification (spec 2026-09-23 D4.2)', () => {
  ...
  it('a token change mid-flight drops the first answer (review #2)', async () => {
    const first = deferred<HostInfo>()
    fetchHostInfo.mockReturnValueOnce(first.promise).mockReturnValue(new Promise(() => {}))
    stop = startHostDaemonIdVerification()
    useHostStore.getState().setRuntime('h1', { status: 'connected' })
  ...
  it('a host deleted and re-added with the same id mid-flight drops the old answer (review #2)', async () => {
    const first = deferred<HostInfo>()
    fetchHostInfo.mockReturnValueOnce(first.promise)
    stop = startHostDaemonIdVerification()
    useHostStore.getState().setRuntime('h1', { status: 'connected' })
    ...
    useHostStore.getState().removeHost('h1')
    useHostStore.setState((s) => ({ hosts: { ...s.hosts, h1 }, hostOrder: [...s.hostOrder, 'h1'] }))
    first.resolve(info(OTHER))
    await flush()
    expect('daemonId' in useHostStore.getState().hosts.h1).toBe(false)
  })

  it('out-of-order completion: only the newest request applies (review #2)', async () => {
    vi.spyOn(console, 'warn').mockImplementation(() => {})
    const first = deferred<HostInfo>()
    const second = deferred<HostInfo>()
    fetchHostInfo.mockReturnValueOnce(first.promise).mockReturnValueOnce(second.promise)
    stop = startHostDaemonIdVerification()
    useHostStore.getState().setRuntime('h1', { status: 'connected' })
    useHostStore.getState().setRuntime('h1', { status: 'reconnecting' })
    useHostStore.getState().setRuntime('h1', { status: 'connected' })
    expect(fetchHostInfo).toHaveBeenCalledTimes(2)
    second.resolve(info(ID))
    await flush()
    first.resolve(info(OTHER))
    await flush()
    expect(useHostStore.getState().hosts.h1.daemonId).toBe(ID)
    expect(selectDaemonIdMismatch(useHostStore.getState(), 'h1')).toBeUndefined()
  })

  it('a stale answer never lets a later sync change of daemonId skip re-verification (review #2)', async () => {
    const first = deferred<HostInfo>()
    const second = deferred<HostInfo>()
    fetchHostInfo.mockReturnValueOnce(first.promise).mockReturnValueOnce(second.promise).mockResolvedValue(info(ID))
    stop = startHostDaemonIdVerification()
    useHostStore.getState().setRuntime('h1', { status: 'connected' })
    useHostStore.getState().setRuntime('h1', { status: 'reconnecting' })
    useHostStore.getState().setRuntime('h1', { status: 'connected' })
