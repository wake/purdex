...
// Since P-D.3 every tmux-session pane is a terminal pane, so the entry-pane
// collector and the detail-target collector describe the same set: the
// popover opens for exactly the panes it renders a detail block for.
describe('entry panes and detail targets agree (P-D.3)', () => {
  it('returns the same panes from both collectors, dead ones included', () => {
    const tab = seedSplitTab([terminal({ sessionCode: 'live1' }), terminal({ sessionCode: 'dead1', terminated: 'tmux-restarted' })])
    expect(collectRenameEntryPanes(tab).map((p) => p.sessionCode)).toEqual(['live1', 'dead1'])
    expect(collectRenameEntryPanes(tab)).toEqual(collectRenameTargets(tab))
  })
})
