...
H2c-2, H2c-3, H2d-* NOT merged — `lib/host-look-migration.ts` and `stores/useShownHostsStore.ts` do not exist).
Every task is TDD (failing test first) and its own commit. File lists are the files each PR touches, counted from
...
```
H2c-2 ─► H2c-3 (joint bump) ─┐
                             ▼
H3a-1 ─────────────────► H3a-2 ─► H3a-3 ─► H3a-4 ─► H3b ─► one bump PR for all five
                             ▲        ▲
H2d-1 ───────────────────────┴────────┘   (file overlap only — serialise, either order; §0.13)
H2d-2 … H2d-6: no file overlap with H3 except the two locale files — run in parallel.
```

- **H3a-2 needs H2c-2 and H2c-3 merged** (not only H2c-1, which is what main has). Today the look store exists and is
  projected in `settings`, but nothing reads it: the selector (`lib/host-look.ts`) still reads `HostConfig` only
  (H2c-2 T1), writers still write `HostConfig` (H2c-2 T2), the transfer still spreads looks into `HostConfig`
  (H2c-3 T2) and New Tab labels do not follow the look store (H2c-3 T3). Until H2c-2, the ONLY thing carrying a rename
  or a colour to another device is the `hosts` section. Retiring it before H2c-2 would silently stop looks from
  syncing — spec decision 3 broken. H3a-2 therefore merges after H2c-3, and the H3 bump comes after (or with) the
  H2c-2 / H2c-3 bump.
- **H3a-1 does not need H2c** (it only removes the guard), but it reopens #1366 on `main` until H3a-2 lands (a `hosts`
  written between the wizard's check and the first pull could remove a host the user was not told about). Harmless
  in a release because all five PRs ship in one bump; the coordinator may prefer to merge H3a-1 right before H3a-2.
- **H2d**: H2d-1 touches `projections.ts` / `.test.ts`, `collector.ts` / `.test.ts`, `apply-to-stores.ts` / `.test.ts`,
  `host-reresolve.integration.test.ts` — all also in H3a-2 / H3a-3. Serialise with H2d-1 (either order, rebase the
  second). If §0.10's marker bump is chosen, its ordinal is "next after whatever `settings` is when H3a-2 merges"
  (6 → 7 before H2d-1, 7 → 8 after). H2d-2 … H2d-6 share only `locales/en.json` / `zh-TW.json` with H3 (merge
  conflicts, no semantic overlap).
- **One bump PR for H3a-1 … H3b.** A released build with only part of H3 would show the wizard's "this pull removes
  these hosts" list while nothing is removed (H3a-2 without H3b), or stall a guarded pull (a guard without the
  retired section — §0.3). The H3 release notes carry spec §5.4's known limitation (§H3-note).

## 0. Where the spec and the code disagree (found while measuring)

Status per item: plain = measurement / plan choice; **NEEDS DECISION** = options side by side, recommendation given,
the plan is written for the recommendation.
...
8. **The pull premise's reason is gone, its check is not (spec silent).** `brokenPullPremise` (`wizard-run.ts:313-318`)
   ...
   (`en.json:1697`). After H3 no host list is matched. → **NEEDS DECISION D3**.
...
10. **An H2-era client that is not yet H3 is locked by nothing (spec decision 7 / §5.4).** Decision 7 says "H2 locks
    ...
    the known limitation limits to hosts-only profiles. → **NEEDS DECISION D1** (a marker bump).
...
- **D1 — lock H2-era pre-H3 clients with a marker bump? (§0.10)**
  (a) no bump: spec as written; a not-yet-upgraded H2-era client keeps syncing `hosts` (tokens included) on every
  profile until it is upgraded. (b) **recommended**: H3a-2 adds `@wire:hosts-retired=1` to `WIRE_MARKERS.settings`
  and bumps `SECTION_SCHEMA_ORDINAL.settings` by one — no projection change; an H2-era client then sees `settings`
  as newer and locks the whole profile (all writes stop, its `hosts` pushes included). Cost: 0 extra files
  (`projections.ts` / `.test.ts` are in H3a-2), one lock-regression test in the style of H2c-1 T2. Limit, to be said
  in the release notes: like every marker it bites only once an H3 client has WRITTEN `settings` to that SOT — a
  marker changes the fingerprint, not the payload hash, so an unchanged `settings` is not re-pushed; the first
  settings edit after the upgrade (or H2d-1's field, if it ships in the same release) writes it. If H3 ships in the
  SAME bump as H2d-1 (which already bumps `settings` for everyone below it), (b) is redundant and (a) is enough.
- **D2 — `deleteHostCascade(hostId, grant, afterCommit)` after the hosts apply goes (§0.6).**
  (a) **recommended**: keep both parameters and their tests; rewrite the doc comment (`host-lifecycle.ts:40-48`) to
  say they exist for a caller that deletes hosts inside its own transaction (spec §6.4 step 6 replace-all, not built)
  and open an issue "H4 replace-all mode missing (spec §6.4 step 6)". (b) remove them (and the grant / afterCommit
  cases of `host-lifecycle.test.ts`) as dead code; a future replace-all re-adds them.
- **D3 — the wizard's pull premise (§0.8).**
  (a) **recommended**: keep `brokenPullPremise` (`wizard-run.ts:313-318`) (a pull resolves the SOT's `d1_…` references through the attach
  host's CLAIMED daemonId too; an unverified / mismatched claim can map them onto the wrong host, and the start layer
  would block the sync anyway — `host-identity-mismatch`), and reword `settings.profile.wizard.pull.master_unverified`
  / `.master_mismatch` (en + zh-TW) to drop "its host list cannot be matched" → "the tabs and settings it names cannot
  be matched to this device's hosts safely". (b) drop the premise; a pull through an unverified host attaches and
  sits blocked until the host verifies.
- **D4 — the 800-line rule for deletion-dominated PRs (§0.12).** H3a-1 (≈ −960 / +30), H3a-3 (≈ −1050 / +150) and
  H3a-4 (≈ −520 / +40) pass the 20-file limit but not an 800-line limit that counts deletions. A test of deleted code
  cannot stay behind in a later PR (it would fail to compile), so a further split only moves production deletions
  apart from test deletions of the same code. (a) **recommended**: accept them — the added lines are far under 800,
  and reviewing a deletion is checking that nothing still calls it (lint / `tsc` / the suites do most of that);
  (b) the coordinator names a different split.

## What stays on the SOT, and what an old client then does

- **New clients never read, write or delete the `hosts` row** (H3a-2). It stays on the daemon, unchanged, with every
...
Files:
1. `spa/src/lib/profile/projections.ts` (`RETIRED_SECTIONS`, `isRetiredSection`; D1 marker + ordinal if (b))
...
15. `spa/src/lib/host-reresolve.integration.test.ts` (`:35` KEYS without `hosts`)
...
- **T1 — `isRetiredSection` and the lock.** Tests: `isRetiredSection('hosts')` true, `'settings'` / `'workspaces'` /
  ...
  **[D1 (b)]** marker array pinned, guard snapshot updated, lock regression: an H2-era shape table (current
...
- **T4 — spec §5.3's never-PUT / never-DELETE list, end to end** (`hosts-retired.integration.test.ts`: real executor,
  collector, `applySectionToStores`, fake daemon; the SOT seeded with a `hosts` row holding a token, written by
  "another client"). With a spy on `api.putSection` / `deleteSection` / `getSection`, for each: (1) push attach;
  (2) pull attach; (3) a `settings` conflict resolved keep-local (restore-local path); (4) resolved keep-SOT; (5)
...
Invariants: no request of any kind names `hosts`; `profileLock` never answers for `hosts`; `tabs.*` / `settings` gate
on `workspaces` only; a host-list change on this device changes no section hash except through references it renames
(H1b / H2c rules); the stash holds no `hosts` payload after the executor starts.
...
hide production); M9 **[D1 (b)]** marker without the ordinal bump (guard snapshot red).
...
8. `spa/src/lib/host-lifecycle.ts` (doc comment `:33-48` per D2)
...
(**[D2 (b)]** adds `host-lifecycle.test.ts` → 11 files.)
...
## H3b — wizard, the guard's store half, the notice, the delete copy (19 files)
...
Tasks:
- **T1 — a pull reads no `hosts` and removes nothing.** Tests (`wizard-run.test.ts`, real stores + mocked api): for a
  pull, `prepareRun` / `recheckBeforeAttach` never call `getSection` (spy: zero calls, any section); a plan has no
  ...
  (a SOT whose `hosts` row lacks one of them). Commit.
...
2. **A new host stays on its device.** On A add air26 (`100.64.0.4:7860`) through the UI. B's Hosts list: still mlab
   only (wait 10 s, reload B, still mlab only). No section rev moved except through step 3's tab.
...
4. **A pull never removes a local host.** B adds air26 itself, through its own UI (its own local id). B's pane from
   step 3 goes live (H1b pass, no push: `tabs.<ws>` / `settings` revs unchanged). Then on B: Settings › 工作台 → stop
...
6. **Names per device, looks per workbench.** A re-adds air26 through the add dialog under the name "air-A". The
   workbench already holds a look for air26's `d1_…` (H2c: a look entry is created only when none exists), so A shows
   the workbench name, not "air-A", and B's host list is unchanged. Rename air26 on B → A's badge and New Tab label
...
9. **[D1 (b)]** A settings edit on A (e.g. tab position) → the SOT `settings` row's ordinal is the new one.
...
## H3-note — for the bump PR's CHANGELOG / release notes
...
- Known limitation (spec §5.4): upgrade every device together. A profile whose sync-host copy holds only a host list
  ...
  there. [D1 (b): an H2-era build is locked once this version has written the profile's settings.]
