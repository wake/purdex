...
### Step 1.2：修正 createWorkspace icon bug + 寫測試

- [ ] 在 `spa/src/types/tab.ts` 修正 `createWorkspace` 加入 `icon?` 參數

**檔案：`spa/src/types/tab.ts`**
...
### Step 1.3：修正 addWorkspace 傳遞 icon
...
預期：新增的全自由制測試 fail（`createDefaultState` 仍回傳 Default workspace、`removeWorkspace` 仍守衛、`insertTab` 不存在），既有測試部分也會 fail。
...
### Step 5.4：替換 SessionsSection.tsx
...
### Step 7.17：完整的 store.ts 的 WorkspaceState interface

為避免遺漏，此處列出完整的 interface：
...
```typescript
        {migrateDialog && (
          <MigrateTabsDialog
            ...
            onMigrate={() => {
              // Move all existing tabs into the new workspace
              tabOrder.forEach((tabId) => {
                useWorkspaceStore.getState().addTabToWorkspace(migrateDialog.wsId, tabId)
              })
              // Set the active tab as the workspace's active tab
              if (activeTabId) {
                useWorkspaceStore.getState().setWorkspaceActiveTab(migrateDialog.wsId, activeTabId)
              }
