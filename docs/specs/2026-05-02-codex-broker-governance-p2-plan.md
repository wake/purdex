...
- Assert: the spawned broker appears in `evaluated`, all three predicates evaluated, `Reason="foreign_quarantine"` and `AnomaliesAdded` contains `AnomalyForeignOwner` (registry empty in test setup, same as mlab pre-spawn-hook). Note: baseline `Kill` value is **not asserted** here because a freshly-spawned idle broker with no active threads, no queued/running jobs, and `nil LastJobUpdatedAt` will have all predicates false and idle=∞ → baseline `Kill=true`. This is correct decision-layer behaviour; the broker is process-alive but spec-orphan.
- **Alive-protection invariant**: NOT covered in integration test (round-4 finding #1: a real broker would need an actively-queued/running codex job to make predicate A true, which requires invoking the broker's thread RPC protocol — out of P2 integration test budget). The invariant is fully covered by unit tests `TestSweepHandler_BrokerKeyApply_AlivePredicate_NoKill` and `TestSweepHandler_BrokerKeyApply_IdleNotExpired_NoKill` using mock predicate evaluation. Integration test acknowledges this coverage split in the test docstring.
- Kill the broker externally (SIGTERM). Wait 2 s. The cwd still exists, so the broker remains foreign (no registry entry) and is now also process-dead.
...
| **Alive-protection invariant coverage gap in integration test** (round-4 finding #1) | A spawned-but-idle broker has all predicates false + `nil LastJobUpdatedAt` → baseline `Kill=true`; making predicate A true in integration would require submitting a real codex thread/job through the broker's RPC, which is out of P2 integration test budget. Coverage split: alive-protection invariant is unit-tested with mocked predicates (Task Q tests above); integration test asserts mass-kill safety on a process-dead foreign broker only. Integration test docstring documents this split so reviewers know where to look. P3 may extend integration to cover the active-broker case once a job-submission helper exists. |
