...
import { hostEndpoint, useHostStore, type HostInfo, type HostRuntime } from '../../stores/useHostStore'
...
export function OverviewSection({ hostId }: Props) {
  ...
  useEffect(() => {
    ...
    const endpoint = atRequest ? hostEndpoint(atRequest) : ''
    fetchInfo(hostId)
      .then((r) => r.ok ? r.json() : null)
      .then((data) => {
        // Also this device's daemon-identity check (spec 2026-09-23 D4.3); guarded by the
        // endpoint captured above, so it holds even after unmount.
        if (data && typeof data.host_id === 'string') {
          useHostStore.getState().observeDaemonId(hostId, data.host_id, endpoint)
        }
