...
- [ ] **Step 1: Write the failing tests.**
  ...
  - S7 + S11 with the real engine: tabs X, Y both `tmux-restarted` on `old111`@`111:1000`, name `dev`, records with an agent. `const sendKeys = vi.fn(() => resume.promise)` where `resume = deferred()` (copy `deferred` from `engine.test.ts`). Start `const run = rebuildPane('h1', 'tX', 'pX', plan, { createSession: async () => session({ code:'new1', name:'dev', tmux_instance:'222:2000' }), sendKeys })` **without awaiting it**. `await waitFor(() => expect(sendKeys).toHaveBeenCalledTimes(1))` — the lock is taken before the create runs, so `lockedBy` alone would not prove the payload lands *during* the resume; then assert `lockedBy` is non-null. Emit the payload `[new1]` inside a synchronous `act` (this is the daemon's create broadcast) → X and Y both still terminated. `await act(async () => { resume.resolve(); report = await run })` → X re-pointed **by the engine** (`report.repointed === true`, `sessionCode === 'new1'`); the lock release fires the subscription → Y revived onto `new1` with its record restamped. Also assert the pass did not touch X: X's `rebuild` reference is the one the engine wrote (capture it right after `await run`, run one more matching payload, `toBe`).
  - S12 with the real batch, two `it`s, each one group of source `p1` + member `p2` on `old111` (use the `content()`/`seedPane` fixture from `batch.test.ts`):
    ...
    (b) resume failed — `sendKeys = vi.fn(async () => { await resume.promise; throw new Error('boom') })`. Start `const run = runBatchRebuild({ createSession, sendKeys })` without awaiting; `await waitFor(() => expect(sendKeys).toHaveBeenCalledTimes(1))`; emit the `[new1]` payload inside synchronous `act` (the create response alone never reaches the session store on a failed resume — `engine.ts` syncs the store only on a successful re-point — so without this payload `p2` has nothing to match); assert the session store holds `new1`, `attachReady` is true, `lockedBy` is `rebuild:batch`, both panes still terminated. Then `await act(async () => { resume.resolve(); report = await run })` → `p1` still terminated with an op `done` + `createdSession` (report on screen), `report.groups[0].members[0].repointed === false`, and `p2` revived onto `new1` by the release pass. Assert `p1`'s op entry still describes its dead binding (`usePaneOperation` semantics: `operations.p1.binding.sessionCode === 'old111'`).
...
Confirmation round `task-mu07sg35-r8z704`: S12(b) needed a deferred resume + an explicit payload (create responses reach the session store only on a successful re-point) — fixed; S11 must wait for `sendKeys` to be called, not merely for the lock — fixed. Confirmed: `withOperationLock` releases in `finally` via a synchronous `set`, and Zustand 5.0.12 dispatches listeners synchronously, so the release-trigger pass completes before `await run` returns; `FakeSocket.emit` → `onmessage` → handler is synchronous up to the pass.
