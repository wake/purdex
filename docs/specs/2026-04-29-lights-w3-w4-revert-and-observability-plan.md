...
# [derive]    verify_passed agent=cc purdex_name=PdxUserPromptSubmit status=running reason= chain_id=...
# [handler]   frame_apply session=X frame_id=... lifecycle=PdxUserPromptSubmit decision=updated_frame chain_id=...
# [handler]   projection_built session=X top_status=running subagents=N pane_id=%X chain_id=...
# [broadcast] session=X has_clients=true decision=broadcasted reason=... raw_event_name=PdxUserPromptSubmit chain_id=...
...
# [derive]    skipped agent=cc purdex_name=BogusEvent reason=event_not_in_catalog chain_id=...
# [handler]   invalid_skip reason=event_not_in_catalog chain_id=...
...
# 預期：trigger/verify/frame/projection 有；**[handler] invalid_skip decision=skipped reason=frame_missing chain_id=...**（per handler.go subagent branch frameMeta.Decision != "updated_frame" 早 return，發生在 projection trace 之後）；無 broadcast log
