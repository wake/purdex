...
9. codex R2 (gpt-5.5, 2026-09-15) follow-up: tickets WS-only, `--addr` normalization, Serve-first signal semantics, HOME-less `Init`, `X-Pdx-Client` CORS — PR <this>. The I11 mount test was renamed to claim only principal attribution; the second turn's `--resume` prerequisites remain covered by step 3a above, not by the fixture.
