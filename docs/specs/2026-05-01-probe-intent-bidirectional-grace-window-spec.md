...
**mlab live verify 結果**（2026-05-02，HEAD `b761244b`，main + J3 無 W6-6）：

| Acceptance | 結果 | 證據 |
|---|---|---|
| **A11 SIGKILL** | ✅ **PASS** | codex CLI pid SIGKILL @ 02:36:14.91 → daemon log `[probe-intent] signal session=j3-verify kind=process_dead newStatus=error applied=true` @ 02:36:15.0xx；end-to-end latency ≤1.5s ≤ 2s。Pre-grace 300ms hold 完成（無 hook 到 → proceeded → applied）|
| **A12 daemon replay** | ✅ **PASS** | daemon kill @ 02:37:22 → 新 daemon log `[probe-intent] start session=j3-verify agent=codex kind=process_dead pane=%60 pid=50939 generation=1` @ 02:37:24（~2s）；replayStatus hydrate state 後 ProbeIntent 自動 re-arm |
| **A9 reject 閃 running < 3/30** | 🟡 **N/A on main + J3 alone**；待 W6-6 ship | J3 在 main 上的唯一 active ProbeIntent Kind 是 ProcessDead（W6-3 ship）。codex permission ask reject 路徑的 race（dialog 消失發 ScreenChanged 與 PdxStop hook race）需要 **W6-6 ScreenChange ProbeIntent** 才會出現；W6-6 spec 在 `lights-w6-6-codex-screen-change` worktree 尚未 merge 到 main。J3 PR 是 W6-6 的前置 infrastructure，A9 quantified gate 將在 W6-6 PR 的 mlab live verify 中量測 |
| **A10 approve latency** | 🟡 **N/A on main + J3 alone**；待 W6-6 ship | 同 A9 — approve 路徑的 ScreenChanged emit-once 需 W6-6 detector，J3 alone 沒可量的對象 |

**結論**：A11 + A12 ProcessDead path 完全 cover；A9 + A10 ScreenChange path 因 W6-6 未 merge 不可在 J3 PR 階段量測（**這是 J3 作為前置 PR 的本質特徵**，非 design defect）。J3 PR ship gate 採 A11 + A12 PASS；A9 + A10 為 W6-6 PR 的 ship gate（W6-6 ship 時走完整 30 次 reject 量化）。
...
- `probeIntentOnDropForSession` reason 擴展加 `"pre-grace"` + `"pre-grace-canceled"`（兩條新 reason 不擴 helper switch case → 無 helper-side counter，由 caller `consumeSignals` 顯式 `.Add(1)` 統一管，per §3.4 R4 修）；既有 reasons (`"stale-callback"` / `"grace"` / `"transition-gate"` / `"error-guard"`) 不動；既有 helper 在 step 3 mapping 回 "" 不 emit reason 是 pre-existing 行為，out of J3 scope
...
- **J3 PR ship gate**：A11 (§3 SIGKILL ≤2s) + A12 (§4 daemon replay) mlab live verify PASS
- **W6-6 PR ship gate**：A9 (§1 reject 30 次 ≥ 28/30 quantified threshold) + A10 (§2 approve latency ≤500ms+300ms) — J3 PR ship 階段標 N/A（W6-6 ScreenChange ProbeIntent 尚未 merge → 無可量的 race 對象），完整 30 次 reject 量化在 W6-6 PR mlab live verify 跑
