...
func TestHandleEvent_DoesNotEmitPathHintForBashTool(t *testing.T) {
	...
	body := `{"tmux_session":"work","tmux_pane_id":"%5","sender_pid":200,"sender_start_time":"Sun Apr 20 01:30:00 2026","event_name":"PreToolUse","raw_event":{"tool_name":"Bash","tool_input":{"command":"ls"}},"agent_type":"cc"}`
	req := httptest.NewRequest("POST", "/api/agent/event", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	m.handleEvent(w, req)
