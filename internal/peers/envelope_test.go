...
// internal/module/peers/module.go and cmd/pdx/peers.go): host_id, ok, error
// (omitted when empty), partial, peers (never null). Any change to this
...
func TestEnvelope_JSON_MatchesP1Shape(t *testing.T) {
	...
	want := `{"host_id":"h","ok":true,"partial":false,"peers":[]}`
	if string(got) != want {
...
func TestEnvelope_JSON_ErrorIncludedWhenSet(t *testing.T) {
	...
	want := `{"host_id":"h","ok":false,"error":"boom","partial":false,"peers":[]}`
	if string(got) != want {
		t.Errorf("Envelope JSON = %s, want %s", got, want)
	}
}
