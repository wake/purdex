# Lights Rebuild — Phase 4a Plan v1.1 (PR-4a-0 OpenCode Hooks Completion)

**Status**: Draft v1.1（response to Codex Round 1 review — 9 must-fix + 9 medium/low/nit applied）
**Date**: 2026-04-27
...
**Audit issue**: [#656 v5.1](https://github.com/wake/purdex/issues/656)
**Round 1 review jobs**: codex `task-mog1gsb0-0idiaa` (audit) + `task-mog1gtot-c79vs7` (plan)
**Related specs**:
...
3. **Phase 4a Slice 6（opencode module 接新 primitive）** 在 alpha.230 上**無法 e2e 驗證** — 因為 plugin template 用 `chat.message` / `session.idle` 等 key 是否仍為 1.14.x current API 未驗證；若 stale，opencode 的 `UserPromptSubmit` / `Stop` 不會被 emit，整條 Running/Idle transition probe 在 opencode 上是 dead code

**方案 A 決議**（2026-04-27 主工作流）：將 hooks-hotfix-plan opencode 收尾工作收編為 Phase 4a 的 **PR-4a-0**，與 lights probe rebuild 同一條開發線。本 plan v1.1 聚焦 PR-4a-0 完整細節；PR-4a-1 / PR-4a-2 僅給大綱 + Slice 6 design-impact checkpoint，待 PR-4a-0 對齊報告產出後再起草下一版 plan。

**Directive 加碼項**（user 2026-04-27）：

> opencode 的 agent hook 因為尚未完全實作，建議**重新完整對齊驗證一次**，也同時確認**是否還需要使用 `chat.message`**。

→ PR-4a-0 不可只做 hooks-hotfix-plan §2.2 OpenCode 子集；必須對 `sst/opencode` v1.14.23 source 做完整 enumeration audit，產出獨立的對齊報告 spec，並覆蓋 `chat.message` / `session.idle` / `session.status` non-idle / `permission.ask` strong hook / `question.asked` 共 5 個關鍵決策。
...
2. **opencode catalog 補完 ignored/unsupported 條目**（§2.2）— 對齊 cc/codex 的完整度
3. **opencode SupportedVersion 報告**（§2.3）— 5 個 nil-error CheckHooks return path 全補（共 7 個 return site，2 個 error path 不帶版本欄）
4. **opencode provenance fixtures**（§2.4）— `testdata/opencode-1.14.23-*`（含 `manifest.json` + `events.json` + 每 template event 的 payload fixture）
5. **opencode plugin template refresh — conditional**（§2.5）— 廣化 skip 條件
...
### 1.3 PR-4a-1 / PR-4a-2 大綱（不在本 plan v1.1 細部，待 PR-4a-0 對齊結果後起草下一版 plan）

詳見 §7。**注意**：§7.3 Slice 6 design-impact checkpoint 是 PR-4a-0 ship 前的 stop/go gate。
...
1. **Strong hooks full list (1.14.23)**
   ...
   - **`permission.ask` strong hook 顯式判定**（new — Round 1 H2 補）：
     - `permission.ask` strong hook（return `'allow'`/`'ask'`/`'deny'`，可 mutate）vs Bus event `permission.asked`（pure observation，不可 mutate）
     - Purdex 目前用 Bus event；strong hook 是否更適合 Waiting trigger（特別是 deny path 是否能讓 status 不進 Waiting）
2. **Bus events full list (1.14.23)**
   ...
   - **`session.idle` 顯式判定**：
     ...
     - vs `session.status({type:"idle"})` 取捨；若上游確認 deprecated 但 emit，是 partial-stale 案例（見 §8 Risk）
   - **`session.status` non-idle 顯式判定**（new — Round 1 H2 補）：
   ...
   - **`question.asked` 顯式判定**（rewritten — Round 1 H1）：
     - v1.14.23 source 直接 `bus.publish(Event.Asked, info)`（不是 v2 SDK only）
     - public docs 未列為 plugin event；這是「source-level 存在但 docs 未公開」案例
     - 決策：是否接受 undocumented Bus event 作為 supported mapping（保留現用 `permission.asked + question.asked → PermissionRequest` 雙路徑）
3. **Catalog 對齊建議**
   - 從上游全清單中，列出 Purdex `events.go` 應補的 ignored / unsupported 條目（具體 Name 字串清單 + 對應 upstream key）
4. **Plugin template 影響**
   - 若 §2.1.1 / §2.1.2 任一判定要換鍵 / 加 filter / 改 payload path / 加 feature detection → 列出具體換鍵建議
...
**`HookEventSpec` 不加新欄位**。upstream key ↔ purdex normalized name 的 mapping 由 `internal/agent/opencode/testdata/opencode-1.14.23-events.json`（§2.4 manifest）保存，testdata 即 SSoT。
...
4. 既有 8 個條目（SessionStart / UserPromptSubmit / SubagentStart / SubagentStop / PermissionRequest / Stop / StopFailure / SessionEnd）**不變 Name 與 Handling**
...
**`events.json` schema**（new — Round 1 C2 / M9，作為 testdata SSoT）：
...
**預期條目數量**：依 §2.1 報告。粗估 OpenCode bus events 28+ + strong hooks 19+ - 已宣告 8 ≈ **30~40 個新 ignored/unsupported 條目**（actual 數量以 events.json 為準）。
...
**7 個 CheckHooks return site，5 個 nil-error path 必須帶版本欄**（fixed — Round 1 L15/AL2）：

| Site (line 約) | Type | 加 SupportedVersion? |
|---|---|---|
| 63 — `cannot find home dir` | error return | ❌ 不加（其他 agent 同 pattern）|
...
├── opencode-1.14.23-manifest.json    ← single-file manifest（new — Round 1 M9）
│   ├ tag / commitSha / version / normalizedVersion
│   ├ auditReport (path)
│   ├ payloadFixtureDir (path)
...
├── opencode-1.14.23-source.md         ← source URLs + commit hash + audit 過程記錄
├── opencode-1.14.23-events.json       ← §2.2 schema，**Commit 2 即建立**（不 defer 到 Commit 4）
└── opencode-1.14.23-payloads/
...
**對應測試**（OC1a）：`plugin_template_test.go` 加 `TestOpenCodeTemplateEventContractsDocumented` — 對每個 plugin 訂閱的 event，從 `payloads/<event>.json` load payload，跑 mock plugin handler，斷言 `pdx hook` 收到的 normalised event payload 含必要欄位。
...
**Skip 條件（廣化 — Round 1 H5）**：當且僅當以下**全部**為真，本 commit 可 skip：
...
2. 所有 payload paths（template handler 從 payload 讀的每個 field）在 1.14.23 schema 仍存在
3. 不需要新增 SDK / 版本 feature detection
4. 不需要新增 `subscribe(filter)` filter
5. 沒有 deprecated-but-supported key 需要切換或雙訂閱（`session.idle` deprecated 例外處理 — 見 §8 Risk M10）
...
4. 若 `session.idle` 雙訂閱（deprecated key + new `session.status` filter）→ 加 dedup logic 防止 double-fire
...
**注意**：實作前先讀 `spa/src/lib/agent-icons.tsx` 確認當前 `getAgentIcon` signature 與 options shape；snippet 內 `/* current API options */` 與 `/* current API */` / `/* variant variation */` 由實作者依 alpha.230 source 填入（avoid stale assumption）。
...
|---|---|---|---|---|
| HC5 | `TestOpenCodeEvents_ClassifyAgainstFrozenManifest` | `internal/agent/opencode/events_test.go` | 從 `testdata/opencode-1.14.23-events.json` load 上游 entries，斷言每 entry 在 `opencodeEventSpecs` 內或被顯式標記 ignored/unsupported 在 manifest 內。**獨立可信 SSoT**，不依賴 audit markdown | red |
| HC5b | `TestOpenCodeEvents_NonInstallableHaveExplicitHandling` | `internal/agent/opencode/events_test.go` | 新加 ignored/unsupported 條目顯式設 `Handling`，不依賴 default fallback | red |
...
| OC1 | `TestOpenCodePluginTemplate_UsesVerifiedEvents` | `internal/agent/opencode/plugin_template_test.go` | **Unconditional**（fixed — Round 1 C1）— 用 `payloads/<event>.json` 跑 mock plugin handler，驗證 spawn `pdx hook` 帶正確 normalised payload。**未換鍵分支**：驗現有 9 個 callback 的 mapping；**換鍵分支**：驗新 mapping 取代舊鍵 | red |
| OC4 | `TestOpenCodeCheckHooks_ReportsSupportedVersion` | `internal/agent/opencode/hooks_test.go` | table tests 跑 5 個 nil-error CheckHooks return path（missing plugin / unmanaged / path resolve fail / managed body drift / fully installed），全部 return 含 `SupportedVersion=1.14.23`；2 個 error path 不檢查版本欄 | red |
| OC5 | `TestOpenCodeCheckHooks_ExceedsSupport` | `internal/agent/opencode/hooks_test.go` | mock `agent --version` return `1.15.0` → `ExceedsSupport=true`；return `1.14.23` → false；return `1.14.0` → false | red |
| OI1 | `returns opencode icon for opencode agent type` | `spa/src/lib/agent-icons.test.tsx` | `getAgentIcon('opencode', ...)` returns defined component | **characterization**（baseline 即綠 — Round 1 L13）|
...
**Test 數量**：9 (含 OC1 unconditional)。OC1 在 Commit 5 skip 時測現有 mapping，Commit 5 執行時測新 mapping。
...
**內容**：§2.1 列的 4 個章節 + 5 個顯式判定（`chat.message` / `session.idle` / `session.status` non-idle / `permission.ask` / `question.asked`） + Catalog 對齊建議 + Plugin template 影響
...
**Red**：HC5 / HC5b / HC5c 失敗（catalog 缺新條目，且 manifest 不存在）

**Green**：

- 建 `internal/agent/opencode/testdata/opencode-1.14.23-events.json`（§2.4 schema） + `opencode-1.14.23-manifest.json`
- 依 Commit 1 audit 報告補 `opencodeEventSpecs` 的 ignored/unsupported 條目
- 加 HC5 / HC5b / HC5c — HC5 從 manifest load expected set 做 exact assertion
...
- `SupportedStatuses()` 不該變動（因為新加條目都 empty `EmitsStatus`），跑 `internal/agent/supported_statuses_test.go` 確認 cc/codex/opencode 三家 supported set 不變
- **events.json 提前到本 commit**（fixed — Round 1 C2 / M8）— 不 defer 到 Commit 4，避免 HC5 的 red/green 缺獨立依據
...
- 加 OC4 table test（5 nil-error path 全綠；2 個 error path skip 版本檢查）
...
**Red**：OC1a / OC1 失敗（payload fixtures 不存在，OC1 沒有 fixture 可跑）

**Green**：

- 建 `internal/agent/opencode/testdata/opencode-1.14.23-payloads/<event>.json`（每 plugin 訂閱 event 一份）
- 建 `opencode-1.14.23-version.txt`（real `opencode --version` 輸出 — for OC5 + manual round-trip 驗證）
- 建 `opencode-1.14.23-source.md`（source URLs + commit hash）
...
**前置**：§2.5 廣化 skip 條件**任一不成立**（key stale / payload path 變 / 需 feature detect / 需 filter / 需 dedup）
...
- 若 §2.5 條件 (5) 雙訂閱情境 → 加 dedup test case
- 同步更新 manifest.json 的 `templateConsumesAt` line numbers
...
- §6 ship gate 對 OC1 的要求**不變**（unconditional）

### Commit 6 — `test(spa): add opencode agent icon characterization coverage`

**Type**: Characterization / coverage commit（**not TDD red** — Round 1 L13）

**Justification**: production icon support 已在 alpha.230 baseline 存在（`agent-icons.tsx` import 並 return `OPENCODE_ICON`）。本 commit 補測試覆蓋率 + 防回歸 guard。
...
## 6. Ship gate（PR-4a-0 — Round 1 H6 加 3 gate）

PR-4a-0 ship 必須全綠：

| 項 | 條件 |
|---|---|
| Audit 報告完整 | Commit 1 文件覆蓋 §2.1 列的 4 章節 + 5 個顯式判定 |
| HC5 / HC5b / HC5c | catalog 完整對齊 manifest（exact set assertion）|
...
| **events.json ↔ events.go exact match**（new — H6.1）| 自動化測試斷言 manifest `installable` set + `ignored`/`unsupported` set 與 `opencodeEventSpecs` 一致 |
| **template consumed keys ↔ payload fixtures exact match**（new — H6.2）| 自動化測試斷言 `plugin_template.go` 訂閱的每個 key 都有對應 `payloads/<event>.json` 且反之 |
| **changed-file boundary check**（new — H6.3）| `git diff --name-only origin/main` 結果 ⊆ §5 Allowed paths；落在 Forbidden paths 即 fail |
...
## 7. PR-4a-1 / PR-4a-2 大綱（Round 1 M11 加 stop/go checkpoint）

### 7.1 PR-4a-1（probe primitive + cc + helper + dev log）大綱

| Slice | 範圍 | 估 LoC | 估 tests |
|---|---|---|---|
| 0 | tmux API 擴展（**`CapturePaneTopLines(target, n)` helper + 內部用 `CapturePaneRange(target, start, endInclusive)`** — Round 1 AM3 inclusive end + helper 防 off-by-one）+ fake executor 對應 | ~40 | 4 |
| 1 | Probe primitive 重構（`WatchFullScreen` / `WatchTopLines` + `watchLoop` + `ScreenChangeEvent`）；移除 `ActivitySignal` enum 的 signal 解讀職責；**watcher ownership 不在 callback 中 delete**（Round 1 AM4 — current code callback 開頭即 delete watcher，continuous design 改由 watch loop 自己管）| ~80 | 6 |
...
| 7 | graceWindow + `PDX_DEV_MODE=1` observation log + **新增 expvar `purdex_probe_*`（不 rename 既有 `purdex_phase35_*`）** | ~30 | 4 |
...
### 7.3 Slice 6 design-impact stop/go checkpoint（new — Round 1 M11）

PR-4a-0 ship 前**必須**產出 `Slice 6 design impact summary`（一段 plan v2 草稿章節），檢驗以下 3 個 trigger pattern 是否仍可靠：
...
3. **Waiting transition trigger**：opencode `PermissionRequest`（從 `permission.asked` + `question.asked`）是否兩條 path 在 1.14.x 上**穩定 emit**？（依賴 §2.1 `question.asked` 判定）

**Stop/Go 規則**：

- **All 3 reliable** → Go：PR-4a-2 Slice 6 設計按 plan 大綱繼續，opencode 與 cc/codex 共用 module orchestrator
...
| Risk | Mitigation |
|---|---|
| §2.1 audit 範圍過大（OpenCode source 變動快，30+ events 全梳理）| 限定 1.14.23 tag，不追 dev branch；audit 用 subagent 平行查（一支 strong hooks / 一支 bus events / 一支 message + question + permission flow），各回 enumerated list 後主 Claude 整合 |
...
| **Partial-stale event keys**（new — Round 1 M10）| `session.idle` 是 deprecated-but-still-emit 的典型；**§2.1 對每個 partial-stale key 必須給出三選一決策**：(a) 保留現用（接受 deprecation 風險）/ (b) 切換到新鍵（可能斷舊 OpenCode 版本兼容）/ (c) 雙訂閱（需要 dedup 防 double-fire，必須加 dedup test case in Commit 5） |
| §2.5 conditional 換鍵牽動 Slice 6 設計 | 本 plan 顯式 declare PR-4a-2 待 PR-4a-0 ship 後再起草 + §7.3 checkpoint 強制檢驗 trigger reliability |
...
| 主 repo 並發 session（feedback_concurrent_session_safety）— Round 1 H7 修正 | 進 worktree 前**先 `git status -s` 確認 clean**；clean 後 `git fetch origin main && git reset --hard origin/main`（alpha.230 對齊）；**若非 clean 則停止並由 user 處理 in-progress work，不可自動覆蓋**。commit 後 push 前再 `git pull --rebase origin main` 一次 |
| §7.3 checkpoint 失敗時 PR-4a-1 進度延誤 | checkpoint 是強制 stop/go gate；若 stop，主工作流必須暫停 PR-4a-1 起草，先處理 design-blocking issue |

---

## 9. LOC 預估（Round 1 M12 重算）

| Commit | 估 LoC | 估 tests | 備註 |
|---|---|---|---|
| 1: audit 報告（純文件）| ~400 (新文件) | 0 | 含 5 個顯式判定章節 + Slice 6 impact summary |
| 2: catalog 補完 + manifest + events.json | **~180~260** (events.go) + **~150~250** (manifest.json + events.json) | 4 (HC5 + HC5b + HC5c + boundary check) | 30-40 entries × 4-6 lines/entry; manifest schema 含 30+ bus + 19+ strong hooks |
| 3: SupportedVersion | ~30 (hooks.go) | 4 (OC4 5-path table + OC5 三 case) | 不變 |
| 4: payload fixtures + version.txt + source.md + OC1/OC1a | ~250~400 (fixtures + helpers) | 6 (OC1a per-event + OC1 per-event) | 9 events × payload fixture + 9 OC1 mock-handler test cases |
| 5: plugin refresh (conditional) | 0~60 | 0~5 (OC1 update + dedup test) | dedup test 增加 |
| 6: SPA icon test | ~30 | 2 (OI1 + OI2) | characterization |

**總計**：**~1040~1430 LoC + 16~21 tests**（含新文件 ~400 LoC + 新 testdata 樹）

純 Go/TS/JSON code 改動：~640~1030 LoC + 16~21 tests。**屬中-大型 PR**。

**拆 PR 門檻**（new — Round 1 M12）：
...
- §6 ship gate 全綠（含 3 個新 gate H6.1/H6.2/H6.3）
- §7.3 Slice 6 design-impact checkpoint 通過（Go 結論）
- PR merged
- 對應 main bump PR ship（VERSION 進到 alpha.231）

**Memory 更新**：

- `kickoff_lights_rebuild.md` 更新「目前進度」段落，記 PR-4a-0 ship 與 §2.1 對齊報告路徑 + §7.3 checkpoint 結論
...
- Codex Round 1 review jobs: `task-mog1gsb0-0idiaa`（audit）+ `task-mog1gtot-c79vs7`（plan）
