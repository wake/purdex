...
**選定方案：Title Bar + Activity Bar + Tab Bar + 4 Region**
...
│ [⚙] │      │      │               │      │      │
├──────┴──────┴──────┴───────────────┴──────┴──────┤
│  host │ session │ status              │ mode │ sz │  ← Status Bar
└─────────────────────────────────────────────────────┘
```

> **命名規則**：Sidebar = outer（全高），Panel = inner（Tab Bar 下方）；Primary = 左，Secondary = 右。
> 區域內的個別內容稱為 **View**（如 SessionsView、FilesView）。

### 佈局元素

| 區域 | 說明 | 可見性 |
|------|------|--------|
| Title Bar | 紅綠燈 + 視窗標題（Electron 專用） | 常駐，最頂部 |
| Activity Bar | 工作區/獨立分頁切換（類 Slack） | 常駐，最左側 |
| Primary Sidebar | 系統級 View 區（全高） | 可折疊/縮減/固定 |
| Tab Bar | 當前工作區的分頁 + 群組操作 | 常駐 |
| Primary Panel | 工作區級 View 區（Tab Bar 下方） | 可折疊/縮減/固定 |
| 內容區域 | 當前分頁的內容渲染（Pane） | 常駐 |
| Secondary Panel | 工作區級 View 區（Tab Bar 下方） | 可折疊/縮減/固定 |
| Secondary Sidebar | 系統級 View 區（全高） | 可折疊/縮減/固定 |
| Status Bar | 當前連線狀態摘要 | 常駐 |
...
// Sidebar = outer（全高，系統級），Panel = inner（Tab Bar 下方，工作區級）
// Primary = 左，Secondary = 右
type SidebarRegion =
  | 'primary-sidebar'    // 左 outer — Activity Bar 右側，全高
  | 'primary-panel'      // 左 inner — Content 左側，Tab Bar 下方
  | 'secondary-panel'    // 右 inner — Content 右側，Tab Bar 下方
  | 'secondary-sidebar'  // 右 outer — 最右側，全高
...
**系統級 View（預設放 Sidebar）：**

| # | View | Phosphor Icon | 說明 |
|---|------|---------------|------|
...
View 可放置在 4 個區域，使用者自由拖曳配置：

> **命名規則**：Sidebar = outer（全高，系統級），Panel = inner（Tab Bar 下方，工作區級）；Primary = 左，Secondary = 右

| 區域 | ID | 垂直範圍 | 預設層級 | 說明 |
|------|-----|----------|----------|------|
| Primary Sidebar | `primary-sidebar` | 全高（與 Tab Bar 並列） | 系統級 | Activity Bar 右側 |
| Primary Panel | `primary-panel` | Tab Bar 下方 | 工作區級 | 內容區左側 |
| Secondary Panel | `secondary-panel` | Tab Bar 下方 | 工作區級 | 內容區右側 |
| Secondary Sidebar | `secondary-sidebar` | 全高（與 Tab Bar 並列） | 系統級 | 最右側 |

每個區域可放置一或多個 View，以圖示分頁切換。配置透過拖曳或設定調整（後期實作）。

### 7.3 三種顯示模式

每個區域有三種顯示模式，可快捷鍵循環切換：

| 模式 | 行為 | 觸發 |
|------|------|------|
| **📌 固定** | 始終展開顯示，不受 context 切換影響 | 手動釘選 |
| **⚡ 預設** | 同側 Sidebar+Panel 智慧切換（見 7.4） | 預設行為 |
| **◁ 縮減** | 收窄為垂直按鈕條，hover / 快捷鍵浮動展開，離開焦點自動收回 | 手動設定 / 預設自動 |

### 7.4 同側智慧切換（預設模式行為）

同一側的 Sidebar 和 Panel 在預設模式下，根據 context 自動切換優先級：

**在工作區分頁時：**
- Panel（目錄、Git 等）→ **展開**（優先）
- Sidebar（Sessions 等）→ **縮減**

**在獨立分頁時：**
- Sidebar（Sessions 等）→ **展開**（優先）
- Panel → **縮減**（或隱藏，無工作區 context）

📌 固定模式的區域不參與自動切換，始終保持展開。

### 7.5 縮減模式細節

- 縮減為窄條（約 24px），顯示垂直文字標籤和圖示
- **Hover**：浮動展開覆蓋在內容區上方（不推擠佈局）
- **快捷鍵**：展開區域，再次按下或焦點離開時收回
...
- 切換工作區（Activity Bar）→ Panel 的目錄/Git/資訊/AI 歷史 View 跟著切換
- 選中獨立分頁 → Panel 縮減，Sidebar 展開
- 每個工作區記憶自己的區域 View 選擇和寬度

### 7.8 Sessions View 細節
