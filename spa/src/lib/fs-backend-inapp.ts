...
import { TreeRevisionMismatchError } from './fs-backend'
...
/**
 * Reserved record holding the monotonic tree revision (2c restore atomicity).
 * Real paths always start with `/`, so this NUL-prefixed key can never collide
 * with a file, never matches a `/buffer/`-prefixed scan (list / listTreeUnder) or
 * replaceTree's `/buffer/` clear, and is never wiped. Its `mtime` field is reused
 * as the monotonic counter.
 *
 * The NUL is written as an escape, never embedded raw: a literal NUL byte makes
 * git classify the whole file as binary, and a file whose diff reads "Binary
 * files differ" drops out of review and out of every text-based merge. The byte
 * itself is unchanged — it is a persisted IndexedDB key.
 */
...
/** Minimal structural type for the idb object store methods the rev helper uses. */
interface RevStore {
  get(key: string): Promise<unknown>
  put(value: StoredFile): Promise<unknown>
}

/** Read the current monotonic revision within a txn (0 if never set). */
async function readRevision(store: RevStore): Promise<number> {
  const rec = (await store.get(REVISION_KEY)) as StoredFile | undefined
  return rec?.mtime ?? 0
}

/** Bump the monotonic revision by one within the caller's txn. */
async function bumpRevision(store: RevStore): Promise<void> {
  const next = (await readRevision(store)) + 1
  await store.put({ path: REVISION_KEY, content: new Uint8Array(0), isDirectory: false, mtime: next })
}
  ...
  implements FsBackend, SupportsUniqueCreate, SupportsMutationEvents, SupportsReplaceTree
{
  ...
  private db(): Promise<IDBPDatabase> {
    return openIDB(DB_NAME, DB_VERSION, (db) => {
      ...
      // and stay safe across future schema version bumps.
  ...
  async mkdir(path: string, _recursive?: boolean): Promise<void> {
    const db = await this.db()
    // Blind overwrite if path already exists (I6). One txn so the revision bump
    // is atomic with the dir write (2c restore guard).
    const tx = db.transaction(STORE, 'readwrite')
    await tx.store.put({
      path,
      content: new Uint8Array(0),
   ...
   */
  /** Current monotonic tree revision (0 if no mutation has happened yet). */
  async getRevision(): Promise<number> {
    const db = await this.db()
    const rec = (await db.get(STORE, REVISION_KEY)) as StoredFile | undefined
    return rec?.mtime ?? 0
  }
  ...
  async replaceTree(
    ...
    try {
      const store = tx.store
      // (0) Revision guard (codex 2c-1 R3, Critical): re-read the stored revision
      // INSIDE this txn before any delete. If a local write committed since the
      // caller captured `expectedRevision`, the counter advanced — abort without
      // mutating so an un-snapshotted concurrent write is never wiped. The check
      // and the wipe share one txn, so there is no lossy window.
      if (expectedRevision !== undefined) {
        const actual = await readRevision(store)
        if (actual !== expectedRevision) {
          throw new TreeRevisionMismatchError(expectedRevision, actual)
        }
      }
      ...
      // A successful replace is itself a mutation — advance the revision so a
      // follow-up restore sees a fresh baseline.
      await bumpRevision(store)
      await tx.done
    } catch (err) {
