...
- **Build the daemon with `make build`**, never bare `go build` — the version comes from ldflags. `make build` writes **`bin/pdx`** and does **not** touch the tracked repo-root `pdx`; only a bare `go build` clobbers that one. Check the built binary as `bin/pdx`, never `./pdx` — the tracked root binary is stale enough to predate the `version` subcommand.
...
- [ ] `make build` — and **only** that. An earlier draft of this bullet said `go build ./... && make build`, which contradicted the Global Constraints two screens up *and* manufactured the exact problem the third bullet then checks for: a bare `go build ./...` writes a `pdx` into the module root, clobbering the tracked binary. Step 1 was creating the failure step 3 detects.
- [ ] `bin/pdx version` reports the ldflags version — **`bin/pdx`, not `./pdx`.** `make build` targets `bin/pdx` (`Makefile:16`) and never touches the repo root. The tracked root `pdx` is stale enough to predate the subcommand entirely: `./pdx version` answers `unknown command: version`, which would read as a build failure and is not one.
- [ ] `go test -race -count=1 ./...` — full suite. **Exactly one failure is expected**, `internal/module/agent`'s `TestConsumeSignals_GraceWindowDrop_RearmsAfterTeardown`. `internal/agent/probe` is a genuine intermittent and often passes; if it reddens, rerun it before assuming anything, and do not treat a *new* probe failure as pre-approved.
