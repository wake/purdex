...
- `SessionPanel` → 遷移為 registered panel（8.5）
...
```
8.1 框架元件 ─→ 8.4 智慧切換
8.2 Registry ─→ 8.5 Sessions Panel
8.3 Store ───→ 8.4 智慧切換
8.1 + 8.2 + 8.3 → 8.6 遷移
```
...
- 8.1：框架元件渲染 + resize 互動
- 8.2：registry CRUD + panel 查詢
- 8.3：store 持久化 + workspace 切換套用
- 8.4：context 切換時 zone 模式變化
- 8.5：Sessions panel 功能回歸
