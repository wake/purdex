...
- Per-event timeout: `SessionEnd` → 3 and `Interrupt` → 3 (both clamped by codex 0.153.4 — measured at startup 2026-09-18: `clamping Interrupt hook timeout to 3s`), everything else stays 5. Encode as a small table, not a special case in the loop.
- Feature flag: `setCodexHooksFeature` writes `features.hooks = true` and deletes `features.codex_hooks` if present. `codexHooksFeatureEnabled` precedence (decided in PR review 2026-09-18): if the canonical `hooks` key is present it decides alone — `true` → enabled, anything else (false, string, number, table) → disabled, never falling through; if `hooks` is absent and `codex_hooks` is present, the same rule applies to it; absent both → enabled (upstream default). "Explicitly false blocks" therefore holds for either key, and a mistyped canonical value surfaces as the "feature flag disabled" issue instead of a false `Installed=true`.
- Install order: `config.toml` is written before `hooks.json`, so a config write failure leaves `hooks.json` untouched (a hooks.json failure after a successful config write leaves only `features.hooks = true`, which is the upstream default). Full two-file rollback is out of scope.
...
`PdxPostToolUse` (LifecycleNone + running) takes the generic narrow-update path **when the sender owns a frame**; `PdxInterrupt` (LifecycleStop) takes the existing Stop path. One guard is added in `applyFrameEvent` (PR review 2026-09-18, attacker A1): a codex `PdxPostToolUse` whose sender has **no** frame (a codex-companion broker proxied on a cc parent, or a late event after SessionEnd/sweep) is reclassified for that dispatch only onto the turn-aware `LifecycleUserPromptSubmit` proxy path — proxy parent → upsert the ref by `turn_id`; no parent → `skipped` / `post_tool_without_proxy_parent` — so it can never materialise a standalone `running` frame. The catalog lifecycle stays None, so the handler error guard is unchanged. Add `frame_ops` cases showing: (a) PostToolUse moves a `waiting` codex frame to `running`; (a') PostToolUse with no frame and no parent creates no row; (a'') PostToolUse with no frame and a proxy parent upserts the ref only; (b) Interrupt moves `running` to `idle` and detaches the proxy ref for that `turn_id`. The error guard (`handler.go:346-371`) is unchanged: `PdxInterrupt` has `LifecycleStop`, so like `PdxStop` it may clear `error` for non-opencode agents.
